...
from .harmonic_functions import HarmonicDetector, kline_to_df
...
def send_alert(title: str, body: str):
    ...
    if not redis_client.exists(body):
        r = httpx.post(
            NOTIFY_URL, data={"text": title, "desp": template.format(body=body)}
        )
            ...
            while retry > 0:
                try:
                    ...
                    patterns, predict_patterns = detector.search_patterns(
                        df, only_last=only_last, last_n=4, plot=plot, predict=predict
                    )
                    break
...
def run(df: pd.DataFrame):
    ...
    while True:
        ...
        search = partial(search_df, detector, df=df)
